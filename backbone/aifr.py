...
class DenseNetAIFR(nn.Module):
    def __init__(self, num_classes=512, input_size=224, drop_rate=0.4):
        ...
        # 注意力模块
        self.attention = AttentionModule(channels=1024)  # 确保通道数与DenseNet的输出匹配

        # 计算DenseNet特征图在全局平均池化之前的尺寸
        final_feature_map_size = (input_size // 32) ** 2  # DenseNet的缩减因子是32

        # 全连接层
        self.flatten = nn.Flatten()
        self.dropout = nn.Dropout(p=drop_rate)
        self.fc = nn.Linear(1024 * final_feature_map_size, num_classes)
    ...
    def forward(self, x, return_age=False):
        ...
        # 展平特征并应用Dropout和全连接层
        out_id = self.fc(self.dropout(self.flatten(x_id)))
        out_id = self.bn(out_id)

        if return_age:
            return out_id, x_id, x_age
...
backbone_dict = {
    ...
    'densenet': partial(DenseNetAIFR, num_classes=512, input_size=224)
